 ...
 * @link      http://sftp.sourceforge.net
 */
...
use sftp\Crypt\Hash;
use sftp\Crypt\RSA;
use sftp\Crypt\Random;
use sftp\File\ASN1;
use sftp\File\ASN1\Element;
use sftp\Math\BigInteger;
...
class X509
{
    ...
    */
    ...
    /**#@+
     * @access public
     * @see \sftp\File\X509::saveX509()
     * @see \sftp\File\X509::saveCSR()
     * @see \sftp\File\X509::saveCRL()
    */
    /**
     * Save as PEM
     *
     ...
     *
     * There's no guarantee \sftp\File\X509 is going to reencode an X.509 cert in the same way it was originally
    ...
    function __construct()
    {
        ...
        */
        ...
        $AdministrationDomainName = array(
            'type'     => ASN1::TYPE_CHOICE,
            // if class isn't present it's assumed to be \sftp\File\ASN1::CLASS_UNIVERSAL or
            // (if constant is present) \sftp\File\ASN1::CLASS_CONTEXT_SPECIFIC
        ...
        $EDIPartyName = array(
            ...
            'children' => array(
                 ...
                 // partyName is technically required but \sftp\File\ASN1 doesn't currently support non-optional constants and
        ...
        $this->oids = array(
            ...
            // the following are X.509 extensions not supported by sftp
    ...
    function saveX509($cert, $format = self::FORMAT_PEM)
    {
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \sftp\File\ASN1::TYPE_IA5_STRING.
           \sftp\File\ASN1::TYPE_PRINTABLE_STRING will cause OpenSSL's X.509 parser to spit out random
           characters.
         */
                    ...
                    case 'id-ce-certificatePolicies':
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                $subvalue = &$value[$j]['policyQualifiers'][$k]['qualifier'];
                                if ($map !== false) {
                                    // by default \sftp\File\ASN1 will try to render qualifier as a \sftp\File\ASN1::TYPE_IA5_STRING since it's
                                    // actual type is \sftp\File\ASN1::TYPE_ANY
    ...
    {
        if (!is_string($extnId)) { // eg. if it's a \sftp\File\ASN1\Element object
            return true;
        }
     ...
     *
     * Reformats a public key to a format supported by sftp (if applicable)
     *
     ...
     * Set public key
     *
     * Key needs to be a \sftp\Crypt\RSA object
     *
     * @param object $key
     * @access public
     * @return bool
     ...
     * Gets the public key
     *
     * Returns a \sftp\Crypt\RSA object or a false.
     *
     * @access public
     * @return mixed
     */
    ...
    function saveSPKAC($spkac, $format = self::FORMAT_PEM)
    {
        ...
        switch ($format) {
            ...
            default:
                ...
                // no other SPKAC decoders sftp will use that same format
     ...
     *
     * @param \sftp\File\X509 $issuer
     * @param \sftp\File\X509 $subject
    ...
    function sign($issuer, $subject, $signatureAlgorithm = 'sha1WithRSAEncryption')
    {
        ...
        // save $tbsCertificate in case there are any \sftp\File\ASN1\Element objects in it
    ...
    function signCSR($signatureAlgorithm = 'sha1WithRSAEncryption')
    {
        ...
        // resync $this->signatureSubject
        // save $certificationRequestInfo in case there are any \sftp\File\ASN1\Element objects in it
    ...
    function signSPKAC($signatureAlgorithm = 'sha1WithRSAEncryption')
    {
        ...
        // resync $this->signatureSubject
        // save $publicKeyAndChallenge in case there are any \sftp\File\ASN1\Element objects in it
    ...
    function signCRL($issuer, $crl, $signatureAlgorithm = 'sha1WithRSAEncryption')
    {
        ...
        // resync $this->signatureSubject
        // save $tbsCertList in case there are any \sftp\File\ASN1\Element objects in it
        $tbsCertList = $this->currentCert['tbsCertList'];
     ...
     * - Key object
     * - \sftp\File\X509 object with public or private key defined
     * - Certificate or CSR array
     * - \sftp\File\ASN1\Element object
     * - PEM or DER string
     *
    ...
    function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            ...
            case $key instanceof X509:
                ...
                return false;
            default: // Should be a key object (i.e.: \sftp\Crypt\RSA).
     ...
     * of sftp may not be unmapped in the next version, so apps that are looking at this OID may not be able
