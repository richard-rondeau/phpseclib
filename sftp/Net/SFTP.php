 ...
 *
 *    $sftp = new \sftp\Net\SFTP('www.domain.tld');
...
class SFTP extends SSH2
{
     ...
     *
     * \sftp\Net\SSH2::exec() uses 0 and \sftp\Net\SSH2::read() / \sftp\Net\SSH2::write() use 1.
     *
    ...
    */
    ...
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        // the order, in this case, matters quite a lot - see \sftp\Net\SFTP::_parseAttributes() to understand why
        ...
        // see \sftp\Net\SFTP::_parseLongname() for an explanation
    ...
    function login($username)
    {
        ...
        */
        ...
        /*
         ...
         in draft-ietf-secsh-filexfer-13 would be quite impossible.  As such, what \sftp\Net\SFTP would do is close the
    ...
    function chdir($dir)
    {
        ...
        // see \sftp\Net\SFTP::nlist() for a more thorough explanation of the following
        $response = $this->_get_sftp_packet();
        switch ($this->packet_type) {
    ...
    function _update_stat_cache($path, $value)
    {
        ...
        foreach ($dirs as $i => $dir) {
            ...
            //  1. a file was deleted and changed to a directory behind sftp's back
     ...
     * Returns general information about a file or symbolic link
     *
     * Determines information without calling \sftp\Net\SFTP::_realpath().
     ...
     *
     * By default, \sftp\Net\SFTP::put() does not read from the local filesystem.  $data is dumped directly into $remote_file.
     * So, for example, if you set $data to 'filename.ext' and then do \sftp\Net\SFTP::get(), you will get a file, twelve bytes
     ...
     * @return bool
     * @access public
     * @internal ASCII mode for SFTPv4/5/6 can be supported by adding a new function - \sftp\Net\SFTP::setMode().
     */
