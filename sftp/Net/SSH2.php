 ...
 *
 *    $ssh = new \sftp\Net\SSH2('www.domain.tld');
...
use sftp\Crypt\Base;
use sftp\Crypt\Blowfish;
use sftp\Crypt\Hash;
use sftp\Crypt\Random;
use sftp\Crypt\RC4;
use sftp\Crypt\Rijndael;
use sftp\Crypt\RSA;
use sftp\Crypt\TripleDES;
use sftp\Crypt\Twofish;
use sftp\Math\BigInteger; // Used to do Diffie-Hellman key exchange and DSA/RSA signature verification.
use sftp\System\SSH\Agent;
...
class SSH2
{
     ...
     *     the other side.
     *
     * @see \sftp\Net\SSH2::_send_channel_packet()
     * @see \sftp\Net\SSH2::_get_channel_packet()
     * @access private
    */
    ...
    */
    ...
    function _key_exchange($kexinit_payload_server)
    {
        ...
        if (class_exists('\sftp\Crypt\RC4') === false) {
            $encryption_algorithms = array_diff(
                $encryption_algorithms,
                array('arcfour256', 'arcfour128', 'arcfour')
            );
        }
        if (class_exists('\sftp\Crypt\Rijndael') === false) {
            $encryption_algorithms = array_diff(
                $encryption_algorithms,
                array('aes128-ctr', 'aes192-ctr', 'aes256-ctr', 'aes128-cbc', 'aes192-cbc', 'aes256-cbc')
            );
        }
        if (class_exists('\sftp\Crypt\Twofish') === false) {
            $encryption_algorithms = array_diff(
                $encryption_algorithms,
                array('twofish128-ctr', 'twofish192-ctr', 'twofish256-ctr', 'twofish128-cbc', 'twofish192-cbc', 'twofish256-cbc', 'twofish-cbc')
            );
        }
        if (class_exists('\sftp\Crypt\Blowfish') === false) {
            $encryption_algorithms = array_diff(
                $encryption_algorithms,
                array('blowfish-ctr', 'blowfish-cbc')
            );
        }
        if (class_exists('\sftp\Crypt\TripleDES') === false) {
            $encryption_algorithms = array_diff(
                $encryption_algorithms,
     ...
     *
     * @param string $algorithm Name of the encryption algorithm
     * @return mixed Instance of \sftp\Crypt\Base or null for unknown
     * @access private
     */
     ...
     * Login
     *
     * The $password parameter can be a plaintext password, a \sftp\Crypt\RSA object or an array
     *
    ...
    function _keyboard_interactive_process()
    {
        ...
        switch ($type) {
            case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                ...
                */
                // maybe sftp should force close the connection after x request / responses?  unless something like that is done
     ...
     *
     * @param string $username
     * @param \sftp\System\SSH\Agent $agent
     * @return bool
     * @access private
     */
     ...
     * Execute Command
     *
     * If $callback is set to false then \sftp\Net\SSH2::_get_channel_packet(self::CHANNEL_EXEC) will need to be called manually.
    ...
    function exec($command, $callback = null)
    {
        ...
        // honestly, if you're transfering more than 2GB, you probably shouldn't be using sftp, anyway.
        ...
        // down.  the one place where it might be desirable is if you're doing something like \sftp\Net\SSH2::exec('ping localhost &').
     ...
     *
     * \sftp\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally closed by the server
